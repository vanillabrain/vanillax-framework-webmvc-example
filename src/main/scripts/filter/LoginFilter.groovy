...
class LoginFilter extends FilterBase {
    ...
    Map<String, Object> preprocess(Map<String, Object> param) throws Exception {
        ...
        if(request != null){
            ...
            if(ignoreUrl){
                ...
                if(one){
                    userinfo = one
                    if(one.tokenValid && one.token && one.token == token){
                        //OK. 정상적인 세션인경우
                        updateUsersTokenUpdateDateByToken([token:token, _userId:userinfo.userId]) // 토큰갱신시간을 변경해준다
                    }else{
                        throw new BaseException("ERR1100","Login Error","invalid token",null)
                    }
                }else{
                    throw new BaseException("ERR1100","Login Error","invalid token",null)
                }
    ...
    def selectUserByToken(param){
        DateUtils.putCurrentDate(param) //currentDate 를 추가한다
