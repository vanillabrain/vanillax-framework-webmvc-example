package vanillax.webmvc.example.servlet;
...
import vanillax.framework.webmvc.servlet.MvcServletBase;
 ...
 */
@WebServlet(name="fileUploadDownload", urlPatterns={"/file/*"})
...
public class FileUploadDownloadServlet extends MvcServletBase {
    ...
    private void doPostUpload(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        boolean onError = false;
        List<IFilter> filterList = null;
        String UPLOAD_DIRECTORY = ConfigHelper.get("cda.file.upload");
        ...
        try{
            ...
            if(request.getContentType().indexOf("multipart") < 0){
                throw new BaseException("ERR1202","잘못된 형식","파일업로드 형식이 아닙니다",null);
            }
            ...
            for (Part part : request.getParts()) {
                ...
                //본래 파일명을 숨길것인지 확인한다
                if(ConfigHelper.getBoolean("hide.upload.filename",false)){
